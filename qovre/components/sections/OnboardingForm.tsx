...
// ── SuccessScreen ─────────────────────────────────────────────────────────────
const WA_MESSAGES = {
  nl: 'Hallo Qovre team, ik heb zojuist mijn aanvraag ingediend en wil graag direct overleggen over mijn project. Kunt u mij meer informeren over het technische analyseproces?',
  en: 'Hello Qovre team, I just submitted my request and would like to discuss my project directly. Could you provide more information about the technical analysis process?',
}

function SuccessScreen({ locale }: { locale: string }) {
  const isNL = locale === 'nl'
  const waHref = `https://wa.me/31647656343?text=${encodeURIComponent(WA_MESSAGES[isNL ? 'nl' : 'en'])}`
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="py-16 text-center">
      ...
      <a
        ...
        className="inline-flex items-center gap-2.5 mt-8 px-6 py-3 bg-green-500 text-white text-sm font-semibold rounded-full hover:bg-green-400 transition-colors shadow-lg shadow-green-500/20"
      >
        ...
        {isNL ? 'Direct contact via WhatsApp' : 'Contact us directly on WhatsApp'}
      </a>

      <div className="mt-3">
        <a
          href={`/${locale}`}
          className="inline-block px-6 py-2.5 text-neutral-500 text-sm hover:text-neutral-300 transition-colors"
        >
